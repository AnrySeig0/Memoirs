"""Insert-only repository for the M1 substrate: sources, sessions, utterances.

`utterances` expose no update/delete by design — append-only is enforced
both here (no API) and at the DB layer (Postgres trigger). These functions
are called by the ingest pipeline, not the review API, so they raise plain
`ValueError` for malformed input rather than the HTTP-mapped domain
exceptions in `claim.py`.
"""
import uuid
